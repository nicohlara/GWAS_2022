...
###Incredibly janky filtering for matrix work, makes model vaguely run but no graph produced
...
###Testing errors, this works but with very limited data
t_ph_group <- group_by(t_ph, Location, Entry) %>% summarize(days_to_head = mean(days_to_head))
	#a tibble: 3958x3. Location, Entry, days_to_head. Both locations
dupLines <- (group_by(t_ph_group, Entry) %>% count() %>% filter(n > 1))$Entry
	#A list 1961 individuals long
t_ph_group <- filter(t_ph_group, !(Entry %in% dupLines))
	#a tibble: 36x3. both Kinston and Raleigh. NO OVERLAP BETWEEN THEM
t_ph_group <- as.data.frame(t_ph_group)
rownames(t_ph_group) <- t_ph_group$Entry
sunVCF_sync <- select.inds(sunVCF, id %in% t_ph_group$Entry)
t_phgn <- t_ph_group[sunVCF_sync@ped$id,]
familyLocCov <- model.matrix(~gsub("-\\d*$", "", t_phgn$Entry) + t_phgn$Location)
testGWAS <- association.test(sunVCF_sync, t_phgn$days_to_head, X = familyLocCov)
